...
function createVis(data) {
    ...
    // *** TO-DO ***
    ageVis = new AgeVis("agevis", allData);
    prioVis = new PrioVis("priovis", allData, metaData);

    d3.select("#start-date").attr("value", dateFormatter(d3.min(allData, d => d.time)));
    d3.select("#end-date").attr("value", dateFormatter(d3.max(allData, d => d.time)));
